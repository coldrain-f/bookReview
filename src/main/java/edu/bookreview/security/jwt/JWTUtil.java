...
public class JWTUtil {
    ...
    private static final long AUTH_TIME = 60000 * 20; // 인증시간 20분
    private static final long REFRESH_TIME = 60000 * 60 * 24 * 7; // REFRESH 시간 7일

    public static String makeAuthToken(User user) {
        return JWT.create()
                .withSubject(user.getUsername()) // 토큰 제목
                .withExpiresAt(new Date(System.currentTimeMillis() + AUTH_TIME)) // 토큰 만료시간
